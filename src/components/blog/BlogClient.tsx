...
export default function BlogClient({ blogPosts }: { blogPosts: any[] }) {
    ...
    return (
        <main className="bg-slate-50 min-h-screen">
            <PageHeader
                title={t('blog_page.header_title')}
                subtitle={t('blog_page.header_subtitle')}
                description={t('blog_page.header_desc')}
            />
                ...
                <div className="container mx-auto px-4">
                    {blogPosts.length === 0 ? (
                        <div className="text-center py-32 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
                            ...
                            <p className="text-slate-400 font-medium mt-2">{t('blog_page.empty_desc')}</p>
                        </div>
                    ) : (
