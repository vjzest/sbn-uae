...
export default function BlogDetailClient({ post }: { post: any }) {
    ...
    return (
        <main className="bg-white min-h-screen pb-20">
            <PageHeader
                title={post.title}
                subtitle={post.category || t('blog_detail.default_category')}
            ...
            <div className="bg-slate-50 border-y border-slate-100 py-6">
                <div className="container mx-auto px-4">
                    <nav className="flex items-center gap-4 text-[10px] font-black uppercase tracking-[2px] text-slate-400">
                        <Link href="/" className="hover:text-[var(--primary-color)] transition-colors">{t('blog_detail.home')}</Link>
                        <FaChevronRight size={8} />
                        <Link href="/blog" className="hover:text-[var(--primary-color)] transition-colors">{t('blog_detail.insights')}</Link>
                        <FaChevronRight size={8} />
                            ...
                            <div className="max-w-full mx-auto">
                                ...
                                <div className="flex flex-wrap items-center gap-6 md:gap-10 mb-12 py-8 border-b border-slate-100">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{post.readTime || t('blog_detail.read_time')}</span>
                                    </div>
                                    <div className="flex items-center gap-3">
                                ...
                                {toc.length > 0 && (
                                    <div className="mb-16 p-8 bg-slate-50 rounded-3xl border border-slate-100 overflow-hidden">
                                        <div className="flex items-center gap-3 mb-6">
                                            ...
                                            <h4 className="text-[11px] font-black uppercase tracking-[3px] text-slate-900">{t('blog_detail.toc')}</h4>
                                        </div>
                                ...
                                <div className="max-w-none break-normal overflow-x-hidden">
                                    ...
                                    <div className="mt-20 p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left overflow-hidden">
                                        ...
                                        <div className="min-w-0">
                                            <div className="inline-block px-3 py-1 bg-[var(--primary-color)]/10 text-[var(--primary-color)] rounded-lg text-[9px] font-black uppercase tracking-widest mb-3">{t('blog_detail.author_insight')}</div>
                                            ...
                                            <p className="text-slate-500 font-bold text-sm leading-relaxed mb-6">
                                                {post.author?.bio || t('blog_detail.author_bio')}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="my-16 p-12 bg-[#0033E7] rounded-[3rem] text-white overflow-hidden relative shadow-2xl group transition-all duration-700 hover:shadow-[0_40px_100px_rgba(0,51,231,0.3)]">
                                        <div className="relative z-10">
                                            <h3 className="text-3xl font-black mb-6 uppercase tracking-tighter text-white">{t('blog_detail.cta_title')}</h3>
                                            <p className="text-blue-100 mb-10 max-w-[500px] font-bold text-sm">
                                                {t('blog_detail.cta_desc')}
                                            </p>
                        ...
                        <div className="lg:col-span-2 hidden lg:block">
                            <div className="sticky top-32 space-y-12">
                                <div className="p-8 bg-slate-50 rounded-3xl border border-slate-100">
                                    <h4 className="text-[10px] font-black uppercase tracking-[3px] text-slate-900 mb-6">{t('blog_detail.categories')}</h4>
                                    <div className="flex flex-wrap gap-2">
                                        {['RCM', 'Billing', 'Compliance', 'Healthcare IT'].map(cat => (
                                            <span key={cat} className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[9px] font-black text-slate-500 uppercase tracking-tighter hover:border-[var(--primary-color)] hover:text-[var(--primary-color)] cursor-pointer transition-all">
                                                {cat}
                                            </span>
                                        ))}
                                    </div>
